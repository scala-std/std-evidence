package std.evidence.variance

import std.data.IsoK
import std.evidence._
import std.evidence.inhabitance.{Inhabited, Proposition}
...
trait Covariant[F[_]] { F =>
  ...
  // FIXME: prove
  def isoMapK[G[_]](f: F IsoK G): Covariant[G] = new Covariant[G] {
    override def apply[A, B](implicit ab: A <~< B): G[A] <~< G[B] = Unsafe.as
  }
