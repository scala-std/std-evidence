...
import std.evidence.internal.{MacroUtil, Unsafe}
import std.evidence._
import std.evidence.variance.Covariant

/**
  * Witnesses that [[A]] is inhabited.
  */
final case class Inhabited[A](run: (A => Void) => Void) { A =>
  import Inhabited._
...
object Inhabited extends InhabitedLowerPriority {
  ...
  // FIXME: prove
  implicit def covariant: Covariant[Inhabited] =
    new Covariant[¬¬] {
      override def apply[A, B](implicit ab: A <~< B): ¬¬[A] <~< (¬¬[B]) =
        Unsafe.as
    }
