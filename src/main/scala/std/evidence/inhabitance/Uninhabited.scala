...
import std.evidence.variance.Contravariant
import std.evidence.internal.MacroUtil

final case class Uninhabited[A](run: A => Void) { notA =>
...
trait UninhabitedLowerPriority {
  implicit def mkUninhabited[A]: Uninhabited[A] =
    macro MacroUtil.mkUninhabited[A]
}
